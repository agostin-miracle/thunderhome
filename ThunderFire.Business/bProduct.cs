...
namespace ThunderFire.Business
{
    ...
    ///Titulo      : SQLDBTools
    ///Version     : 1.3.0.0
    ///Data        : 27/03/2022 20:00
    ///Alias       : product
    ///Descrição   : Products
    ///</remarks>
    public partial class ProductDao : BusinessBase, IProduct
    {
        ...
        /// <returns>int</returns>
        public ExecutionResponse Insert(Product model)
        {
            ...
            using (IDbConnection _conn = ConnectionFactory.GetConnection())
            {
                try
                {
                    ...
                    if (RETURN_VALUE > 0)
                    {
                        ...
                        respond.MessageToUser = "PRODUTO INCLUIDO COM SUCESSO";
                        _errormessage = "";
                    }
                    if (RETURN_VALUE == -1)
                    {
        ...
        public ExecutionResponse Update(Product model)
        {
            ExecutionResponse respond = new ExecutionResponse();
            int RETURN_VALUE = 0;
            int _AUDNUM = 0;
            this.ProcessCode = 10;
            this.HasError = false;
            Product ModelAud = Select(model.CODPRO);
            ...
            using (IDbConnection _conn = ConnectionFactory.GetConnection())
            {
                try
                {
                    ...
                    if (RETURN_VALUE > 0)
                    {
                        ...
                        _AUDNUM = WriteAuditing.Insert(audit);
                        respond.Logged = _AUDNUM > 0;
                        respond.MessageToUser = "PRODUTO ALTERADO COM SUCESSO";
                        _errormessage = "";
                    }
                    if (RETURN_VALUE == -1)
                    {
        ...
        public Product Select(System.Int32 pCODPRO)
        {
            this.Found = false;
            ...
            using (IDbConnection _conn = ConnectionFactory.GetConnection())
            {
                try
                {
                    RETURN_VALUE = _conn.Query<Product>(sql: "PRCADPROSEL", param: new
                    {
                        CODPRO = pCODPRO
