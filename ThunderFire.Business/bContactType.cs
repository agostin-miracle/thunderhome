...
_errormessage="";
    ...
    ContactType RETURN_VALUE=null;
                    ...
                    try
                    {
            RETURN_VALUE  = _conn.Query<ContactType>(sql:"PRTIPCTOSEL", param:new {TIPCTO=pTIPCTO
    ...
    public List<ContactType> List()
        ...
        this.Found=false;
            this.ProcessCode= 0;
    List<ContactType> RETURN_VALUE=null;
                    using (IDbConnection _conn = ConnectionFactory.GetConnection())
                    {
                    try
                    {
            RETURN_VALUE = _conn.Query<ContactType>(sql:"PRTIPCTOSEL", param:new {},  commandType: CommandType.StoredProcedure, commandTimeout: 120).ToList();
                    this.Found = true;
                    }
                    catch (Exception Error)
                    {
