...
_errormessage="";
    ...
    UserType RETURN_VALUE=null;
                    ...
                    try
                    {
            RETURN_VALUE  = _conn.Query<UserType>(sql:"PRTIPUSUSEL", param:new {TIPUSU=pTIPUSU
    ...
    List<UserType> RETURN_VALUE=null;
                    using (IDbConnection _conn = ConnectionFactory.GetConnection())
                    {
                    try
                    {
            RETURN_VALUE = _conn.Query<UserType>(sql:"PRTIPUSUSELALL", param:new {},  commandType: CommandType.StoredProcedure, commandTimeout: 120).ToList();
                    this.Found = true;
                    }
                    catch (Exception Error)
                    {
