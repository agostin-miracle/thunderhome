...
//            }
        ...
        {
            string file = @"D:\REPTHUNDER\ThunderHome\DbScripts\Docs\access.xml";
            ...
            if (nodes != null)
            {
                ...
                for (int i = 0; i < nodes.Count; i++)
                {
                    ...
                    string _sysord = nodes[i].SelectSingleNode("@order").Value;
                    ...
                    sql += string.Format("INSERT INTO TBSYSFUN (SYSAPL,SYSTBL,SYSROL, SYSMTH,SYSPRC,SYSDSC) " +
                        "VALUES ({0},{1},{2},'{3}','{4}','{5}')", _sysprf, _systbl, _sysord, _sysmth, _sysprc, _sysdsc) + Environment.NewLine;
        ...
        static string ComposeTable299()
        {
            string file = @"D:\REPTHUNDER\ThunderHome\DbScripts\Docs\tables.xml";
            XmlDocument doc = new XmlDocument();
            doc.Load(file);
            string sql = "";
            var root = doc.SelectSingleNode("/tables");
            XmlNodeList nodes = root.SelectNodes("//item");
            if (nodes != null)
            {
